...
| [office_attendance_cleanup.js](office_attendance_cleanup.js) | 主动终态查询、统一收尾归属、终态或三分钟后返回桌面 |
| [office_time.js](office_time.js) | 共用北京时间格式、日志前缀、日期与窗口计算 |
...
日期、星期和是否循环由 AutoJs6 的主任务决定；脚本 CONFIG 描述主任务运行当天的时间窗口。脚本窗口显式按北京时间解析，不能跨午夜；AutoJs6 手工创建的周期主任务仍受手机系统时区控制，应保持系统时区为北京时间。
...
仅当 `verifyScheduledAttendance: true` 且随机计划目标在北京时间上午（12:00 前）时接入，核验类型固定为 `OnDuty`；规划、下午计划和普通 `direct` 运行不创建本地核验任务。手机按北京时间使用。已启动但提前／过期跳过、动作失败或中断的子任务也尽量保存事实；本功能不检测根本未启动的子任务。
...
主动远程任务和被动随机任务**本次成功请求启动过钉钉**后，在取得任意服务端终态时返回桌面，包括 `succeeded`、`already_completed`、`failed`、`expired` 和 `unconfirmed`。如果尚未取得终态，则在启动请求成功的 **180 秒后**由本地收尾线程请求桌面；事实补报和官方核验继续进行，返回桌面不等于打卡成功。

保留现有启动后保亮 **15 秒**：不需要为等待核验一直保亮，也不会为了回桌面再次亮屏。不检查当前前台应用，终态或超时到来时直接请求系统桌面。使用 `ACTION_MAIN + CATEGORY_HOME + FLAG_ACTIVITY_NEW_TASK`，不按 Back、不清空任务、不杀后台、不强制停止钉钉。已在测试设备验证屏幕持续关闭时任务栈可以切到桌面，钉钉进程保持不变；支持情况取决于系统及锁屏配置，其他设备需单独验证。

- **被动任务**复用 `office_attendance_queue.js` 已取得的终态，不重复查询官方 API。未接入核验或事实队列保存失败的随机动作，在确实启动钉钉后仍有三分钟收尾。
- **主动任务**由手机额外以只读 GET 查询现有 `/api/devices/{device_id}/attendance/tasks/{task_id}`，正常每 5 秒尝试一次、失败后 30 秒重试；实际频率还受接收器其他请求影响。若创建时已发现上班卡而根本未下发动作，不产生新的桌面请求。
- **三分钟检查**由接收器独立线程每秒执行，不联网；网络长轮询、回执重试不能阻止截止检查。Android 后台调度和正在进行的设备动作可能推迟实际执行，接收器被终止期间不会运行检查，重启后恢复持久化待办。
- **不打断新任务**：新动作取得 `screen-action` 锁后先保存新的归属，使旧收尾失效。读取网络响应后再次检查归属，旧任务晚到结果不能让新任务返回桌面；普通 `direct` 也使旧收尾失效，但不自动收尾自身。
- **防止重复**：最近一次收尾保存在 `.office-mcp/attendance-cleanup.json`。发送 Home 前先保存 `attempting`，成功发送后保存 `requested`；重启不重发已请求或中断于尝试阶段的桌面动作。请求异常记为 `failed`，不无限重试、不改变 Task 考勤结果。升级不会扫描旧 `done` 回执补做历史桌面动作。

动作归属无法落盘时停止本次设备动作并记录 `HOME_STORAGE_ERROR`，避免旧收尾在稍后切走未登记的新任务。应用已启动后的收尾／日志故障单独记录；本地核验结果继续归档。正常日志如下：

| 日志 | 含义 |
|---|---|
| `HOME_PENDING` | 钉钉启动请求成功，已登记收尾归属和三分钟截止 |
| `VERIFY_RESULT` | 服务端终态摘要；主动任务追加到 `office_remote_listener.js.log`，被动任务仍写原入口日志 |
| `HOME_REQUESTED` | 已向 Android 发送桌面请求，原因是 `task_terminal` 或 `verification_wait_timeout`；未做视觉核验 |
| `HOME_FAILED` / `HOME_STORAGE_ERROR` | 系统请求或本地收尾状态失败，不能据此判断考勤失败 |
...
启用远程功能、本地核验、远程开关和桌面收尾时运行一次 `office_remote_listener.js` 并保持常驻，共用时间、动作、队列、控制和收尾模块不需要单独运行。后端需支持现有设备 Task 查询、本地上报及开关同步接口；本次返回桌面没有新增后端接口。将包括 `office_time.js` 的七个 `.js` 文件更新到实际入口目录，保留配置、已登记计划和 `.office-mcp`；确认原定时入口没有正在执行，停止旧接收器后更新并启动新版，已运行实例不会热更新。新增模块应先于依赖它的入口更新。不得通过手动运行定时入口来验收网络链路，以免触发设备动作。设备需具备无需人工解锁即可打开钉钉的条件，并保持 AutoJs6、ZeroTier 和钉钉所需的后台及网络能力。
...
所有业务日志和计划时刻经 `office_time.js` 输出 `yyyy-MM-ddTHH:mm:ss+08:00`，仅展示到秒，内部毫秒数值不变。旧响应／待归档结果中的其他时区时间也在写日志前规范化。手机持久化计划、设备上报、租约和截止计算继续使用同一 Unix 毫秒数值，不平移八小时；服务端向 MCP 展示本地执行事实时转为可读字段。详细规则见 [时间规范](../docs/time-conventions.md)。
