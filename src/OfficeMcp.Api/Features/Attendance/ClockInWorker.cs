...
/// <summary>统一恢复和核验远程及本地考勤任务，HTTP 调用结束不影响任务继续处理。</summary>
    ...
    IOptions<DeviceCommandOptions> devices, IOptions<ClockInOptions> options, ILogger<ClockInWorker> logger) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                foreach (var job in await store.ActiveAsync(ClockInService.Kind, stoppingToken))
                {
                    ...
                    catch (Exception exception)
                    {
                        ...
                        logger.LogError("考勤任务处理异常，任务：{TaskId}，来源：{Source}，类型：{ErrorType}", job.Id, job.Source, exception.GetType().Name);
                    }
                }
            }
            ...
            {
                logger.LogError("考勤任务后台处理异常，类型：{ErrorType}", exception.GetType().Name);
            }
